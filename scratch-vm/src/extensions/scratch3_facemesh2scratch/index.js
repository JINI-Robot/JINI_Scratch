...
const Message = {
  ...
  getX: {
    'ja': '[PERSON_NUMBER] 人目の [KEYPOINT]',
    'ja-Hira': '[PERSON_NUMBER] にんめの [KEYPOINT]',
    'en': '[KEYPOINT] x of person no: [PERSON_NUMBER]',
    'ko': '[PERSON_NUMBER]번 사람의 [KEYPOINT] X 좌표'
  },
  getY: {
    'ja': '[PERSON_NUMBER] 人目の [KEYPOINT]',
    'ja-Hira': '[PERSON_NUMBER] にんめの [KEYPOINT]',
    'en': '[KEYPOINT] y of person no: [PERSON_NUMBER]',
    'ko': '[PERSON_NUMBER]번 사람의 [KEYPOINT] Y 좌표'
  },
  peopleCount: {
    'ja': '人数',
    'ja-Hira': 'にんずう',
...
class Scratch3Facemesh2ScratchBlocks {
    ...
    get KEYPOINT_MENU () {
      let keypoint_menu = [];
      keypoint_menu.push({text: Message.nose[this._locale], value: String(5)});
      keypoint_menu.push({text: Message.upperLip[this._locale], value: String(13)});
      keypoint_menu.push({text: Message.lowerLip[this._locale], value: String(16)});
      keypoint_menu.push({text: Message.leftEye[this._locale], value: String(387)});
      keypoint_menu.push({text: Message.rightEye[this._locale], value: String(160)});
    ...
    getInfo () {
        ...
        return {
            id: 'facemesh2scratch',
            name: Message.name[this._locale],//'Facemesh2Scratch',
    ...
    getX (args) {
      ...
      if (this.faces.length != 0 && this.faces[personNumber].keypoints[keypoint]) {
        let x = 0;
        if (this.runtime.ioDevices.video.mirror === false) {
          x = -1 * (240 - this.faces[personNumber].keypoints[keypoint][0] * this.ratio);
        } else {
          x = 240 - this.faces[personNumber].keypoints[keypoint][0] * this.ratio;
        }
        return Math.round(x);
      } else {
        return "";
    ...
    getY (args) {
      ...
      if (this.faces.length != 0 && this.faces[personNumber].keypoints[keypoint]) {
        let y = 180 - this.faces[personNumber].keypoints[keypoint][1] * this.ratio;
        return Math.round(y);
      } else {
        return "";
      }
