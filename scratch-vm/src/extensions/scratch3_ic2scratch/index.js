...
const Message = {
  ...
  when_received_block: {
    ...
    'en': 'when received detection candidates',
    'ko': '사물인식 결과를 받을때',
    'zh-cn': '收到分类结果时'
  },
  result1: {
  ...
  set_classification_interval: {
    ...
    'ko': '[CLASSIFICATION_INTERVAL]초로 인식간격 조정',
...
const AvailableLocales = ['ko','en', 'ja', 'ja-Hira', 'zh-cn'];
...
class Scratch3ImageClassifierBlocks {
  ...
  getConfidence1() {
    return String(Math.round(this.results[0]['confidence'] * 100)) + '%';
  }

  getConfidence2() {
    return String(Math.round(this.results[1]['confidence'] * 100)) + '%';
  }

  getConfidence3() {
    return String(Math.round(this.results[2]['confidence'] * 100)) + '%';
  }
