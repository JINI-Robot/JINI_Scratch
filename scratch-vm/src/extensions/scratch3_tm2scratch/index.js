...
// 테스트 URL
// 이미지 분류: https://teachablemachine.withgoogle.com/models/ciGcfEvZl/
// 음성 분류 :  https://teachablemachine.withgoogle.com/models/mikHU25_x/
const Message = {
    image_classification_model_url: {
        'ja': '画像分類モデルURL[URL]',
        'ja-Hira': 'がぞうぶんるいモデル[URL]',
        'en': 'object classification model URL [URL]',
...
class Scratch3TM2ScratchBlocks {
    ...
    imageLabelConfidence (args) {
        ...
        return (entry ? String(Math.round(entry.confidence * 100)) + '%' : String(0));
    }
    ...
    soundLabelConfidence (args) {
        if (!this.soundProbableLabels || this.soundProbableLabels.length === 0) return String(0);
